...
import sys,re,json,urllib,urlparse,random,datetime,time,logging,xbmcaddon
...
Addon = xbmcaddon.Addon('plugin.video.20aptest')
...
class sources:
    ...
    def getSources(self, title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, quality='HD', timeout=30):
        progressDialog = control.progressDialog if control.setting('progress.dialog') == '0' else control.progressDialogBG
        progressDialog.create("{0}".format(control.addonInfo('name')), '')
        progressDialog.update(0)
    ...
    def sourcesFilter(self):
        ...
        if debrid_only == 'false' or  debrid.status() == False:
            ...
            filter += [i for i in self.sources if not i['source'].lower() in self.hostprDict and (i['debridonly'] == False or 'magnet:'  in i['url']) ]
        ...
        for i in range(len(self.sources)):
            ...
            if d or 'magnet:' in self.sources[i]['url']: 
    ...
    def sourcesResolve(self, item, info=False):
        if 1:#try:
            ...
            if 'magnet' in url:
                ...
                u = url =rd.singleMagnetToLink(url)
            else:
            ...
            if not local:
                ...
                for part in url.split(' , '):
                    ...
                    if not d == '':
                        ...
                        logging.warning(u)
                        if Addon.getSetting("torrents_en")=='true':
                            uni=True
                        else:
                            uni=False
                        hmf = resolveurl.HostedMediaFile(url=u, include_disabled=True, include_universal=uni)
    ...
    def sourcesDialog(self, items):
        try:
            ...
            block = None
            if 'magnet' in items[0]['source']:
                self.sourcesResolve(items[0])
                self.selectedSource = items[0]['label']
            else:
                for i in range(len(items)):
                    try:
                        if items[i]['source'] == block: raise Exception()
                        ...
                        for x in range(3600):
                            ...
                            k = control.condVisibility('Window.IsActive(virtualkeyboard)')
                            if k: m += '1'; m = m[-1]
                            if (w.is_alive() == False or x > 60) and not k: break
                            k = control.condVisibility('Window.IsActive(yesnoDialog)')
                            if k: m += '1'; m = m[-1]
                            if (w.is_alive() == False or x > 60) and not k: break
                            time.sleep(0.5)
    ...
    def getConstants(self):
        self.itemProperty = 'plugin.video.darkdragon.container.items'

        self.metaProperty = 'plugin.video.darkdragon.container.meta'
        
        #scraperSetting = control.setting('module.provider')
        scraperSetting = 'Default'
        try:
