...
class System(object):
	...
	GaiaAddon = 'plugin.video.20aptest'
	...
	def restart(self, sleep = True):
		...
		System.execute('RunAddon(%s)' % 'plugin.video.20aptest')
	...
	@classmethod
	def info(self, value):
		return xbmcaddon.Addon('plugin.video.20aptest').getAddonInfo(value)
	...
	def window(self, action = None, parameters = {}, command = None, sleep = None, refresh = False):
		if command == None:
			...
			if not parameters == None and not parameters == '' and not parameters == {}:
				...
				parameters = ''
			command = '%s%s/%s' % (System.PluginPrefix, 'plugin.video.20aptest', parameters)
...
SettingsAddon = xbmcaddon.Addon('plugin.video.20aptest')
...
class Settings(object):
	...
	def getInteger(self, id, raw = False, cached = True):
		if id=='scraping.providers.timeout':
				return 10
		elif id=='scraping.providers.pages':
				return 3
		value = self.get(id = id, raw = raw, cached = cached)
...
class Media(object):
	...
	NameSeasonLong = 'season'#xbmcaddon.Addon('plugin.video.20aptest').getLocalizedString(32055).encode('utf-8')
	NameSeasonShort = NameSeasonLong[0].upper()
	NameEpisodeLong = 'S'#xbmcaddon.Addon('plugin.video.20aptest').getLocalizedString(33028).encode('utf-8')
